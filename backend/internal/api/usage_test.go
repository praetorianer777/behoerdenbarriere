...
func TestEndpointsAreCountedByPatternNotByPath(t *testing.T) {
	rec := &fakeRecorder{}
	srv := NewServer(&fakeDB{agencies: []store.AgencyListing{}}, "", "").WithUsage(rec)
...
func TestPageViewComesFromTheHeader(t *testing.T) {
	rec := &fakeRecorder{}
	srv := NewServer(&fakeDB{}, "", "").WithUsage(rec)
...
func TestAPIWorksWithoutACounter(t *testing.T) {
	rec := do(t, NewServer(&fakeDB{}, "", ""), http.MethodGet, "/api/v1/agencies")
...
func TestViewAnswersEmpty(t *testing.T) {
	srv := NewServer(&fakeDB{}, "", "").WithUsage(&fakeRecorder{})
