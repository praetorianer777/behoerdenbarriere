...
export function Statistics() {
	...
	if (usage.isError)
		return <LoadError what="Die Nutzungsstatistik" error={usage.error} />;
	...
	return (
		<div className="max-w-3xl">
			<h1 className="text-3xl font-bold">Nutzung dieser Seite</h1>
			<p className="mt-2 text-slate-700">
				Wir verlangen von Behörden Offenheit, also legen wir unsere eigenen
				Zahlen offen. Zeitraum: {formatDate(data.since)} bis{" "}
			...
			{data.days.length === 0 ? (
				...
				<table className="mt-3 w-full border-collapse bg-white text-left">
					<caption className="sr-only">
						Besuche und Seitenaufrufe je Tag
					</caption>
					<thead>
			...
			<p className="mt-2">
				Gezählt wird in unserer eigenen API, nicht mit einem fremden Skript. Es
				gibt keine Cookies, keine Kennung im Browser und keine Daten, die das
				Haus verlassen. Gespeichert werden ausschließlich Tageszähler der Form{" "}
				<em>Tag, Art, Schlüssel, Anzahl</em> — also zum Beispiel „am 19.09.
				wurde die Seite <code>/dashboard</code> 42-mal aufgerufen“. Ein
				einzelner Aufruf ist darin nicht mehr enthalten.
			</p>
			<p className="mt-4">
				Ein Besuch wird über einen Hash aus IP-Adresse und Browserkennung
				erkannt. Der Schlüssel dieses Hashes wird täglich neu zufällig gezogen,
				liegt nur im Arbeitsspeicher und wird nirgends gespeichert. Damit lässt
				sich der Hash weder zurückrechnen noch über Tage hinweg zusammenführen:
				dieselbe Person an zwei Tagen ergibt zwei Besuche. Die Tageswerte sind
				deshalb eine Obergrenze und keine Personenzahl. Die Hashes selbst werden
				nach {windowDays} Tagen gelöscht.
			</p>
			<p className="mt-4">
				Nicht gespeichert werden: IP-Adressen, Browserkennungen, woher jemand
				kommt, und alles, was zu einer einzelnen Person gehört. Offensichtliche
				Bots zählen nicht mit, und die Endpunkte der Statistik selbst zählen
				sich nicht.
			</p>
		</div>
	);
